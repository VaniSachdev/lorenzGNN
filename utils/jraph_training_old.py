...
# @jax.jit
def train_step_fn(
    state: train_state.TrainState, # included net.fn, params, and optimizer 
    input_graph: jraph.GraphsTuple,
    target_graphs: Iterable[jraph.GraphsTuple],
    n_steps: int = 1,
    # rngs: Dict[str, jnp.ndarray] = None, 
    # # TODO: add rngs arg to state.apply_fn later
...
def train(net, cfg, data_dict_lists, epochs, n_rollout):
    """ Execute model training and evaluation loop.

        Modified from flax GNN training example. 
        ...
        Returns:
            The train state (which includes the `.params` and '.net').
    ...
    train_dataset = data_dict_lists["train"]
    val_dataset = data_dict_lists["val"]
    sample_graph = train_dataset['inputs'][0][0] # 0th window, 0th point

    # Create and initialize the network.
    logging.info('Initializing network.')
    rng = jax.random.key(0)
    rng, init_rng = jax.random.split(rng)
    # TODO the flax example creates a separate net just for initialization, and it's deterministic, while the model net is not deterministic. why? 
    params = jax.jit(net.init)(init_rng, sample_graph)
    ...
    # Create the training state.
    state = train_state.TrainState.create(
        apply_fn=net.apply, params=params, tx=optax.adam(1e-4)
    )
    initial_step = int(state.step) + 1
    ...
    logging.info('Starting training.')
    train_metrics = None
    for step in range(initial_step, train_steps + 1):
        # Split PRNG key, to ensure different 'randomness' for every step.
        rng, dropout_rng = jax.random.split(rng)
        ...
        # with jax.profiler.StepTraceAnnotation('train', step_num=step):
        graphs = jax.tree_util.tree_map(np.asarray, next(train_iter))
        # each batch in the train dataset is a jax pytree
        # this is a higher-order function that maps a function to all the elements in a pytree 
