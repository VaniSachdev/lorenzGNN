...
def get_config():
    ...
    config.sample_buffer = -1 * (
        config.input_steps + config.output_delay + config.output_steps - 1
    ) # so samples are overlapping and consecutive
    ...
    config.edge_features = (32, 16, 8) # the last feature size will be the number of features that the graph predicts
    config.node_features = (32, 64, 32, 2)
